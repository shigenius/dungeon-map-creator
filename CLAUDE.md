...
**機能改善**
- [ ] サンプルダンジョンプロジェクトのインポート機能 (高優先度)
  - [ ] 複数階層を持つ実際のダンジョン形式のサンプルプロジェクトを作成
  - [ ] 新規プロジェクト作成モーダルにサンプルインポートボタンを追加
  - [ ] サンプルプロジェクトのJSONデータを準備（壁、床、イベント、装飾を含む）
  - [ ] インポート機能の実装（プロジェクト作成と同時にサンプルデータを読み込み）
  - [ ] MCP Playwrightでの動作確認（サンプルプロジェクト作成・表示・編集）
- [x] マルチフロア管理機能の実装
