...
- [ ] スポイトツールは削除でOK
- [ ] マップ検証機能は削除でOK
- [ ] すべてのIDを持っているオブジェクトは、新規作成時にUUIDを用いてデフォルトのIDを入力しておく
- [ ] カスタム床タイプ編集
  - [ ] エフェクトにカスタムプロパティを追加
  - [ ] エフェクトタイプテレポートのときにフロアIDを入力できるようにする
- [ ] カスタム壁タイプ編集
  - [ ] 動作設定にカスタムプロパティを追加
  - [ ] 動作タイプテレポートのときに、テレポート先の座標を入力できるようにする。テレポート先のフロアIDを入力できるようにする
- [ ] JSONインポート/エクスポート機能のバグ
  - [ ] カスタム床タイプやカスタム壁タイプが保存されていない。JSONを見るとセルのfloorIDにはカスタム床タイプのIDが入っているが、インポートするとカスタム床タイプは反映されていない。壁も同様
  - [ ] 後述のカスタム装飾タイプも同様に保存できるようにすること
- [ ] デフォルトで用意してある床も編集モーダルを開けるようにする
  - [ ] デフォルトで用意してある床の設定をユーザーが確認できるようにする
- [ ] デフォルトで用意してある壁も編集モーダルを開けるようにする
  - [ ] デフォルトで用意してある壁の設定をユーザーが確認できるようにする
- [ ] デフォルトで用意してある床も複製できるようにする
- [ ] イベントが現状床にのみ配置できないので、壁にも配置できるような設定にする
  - [ ] セル表示上は今のままでいいが、イベント編集モーダルから東西南北または真ん中のいずれか一つに配置できるようにする
  - [ ] 真ん中に表示しているときはイベントの向きを設定できるようにする（東西南北、または未指定）
- [ ] フロアの追加時に、フロアIDの入力を行えるようにする。
- [ ] セルの表示をスライドできるようにする（特にズーム時とか）
- [ ] イベント編集モーダルのバリデーションやバリデーション警告：N件の表示は要らない
- [ ] カスタム装飾タイプを追加できるようにする
- [ ] デフォルトの装飾タイプも編集モーダルを開けるようにすること
- [ ] 矩形ツールで、SHIFTキーを押しているときは矩形を赤色に表示する
- [ ] 矩形ツールで、壁を置くときは矩形の線が引かれている箇所のみ壁を配置する。現在は矩形の中のすべての壁が配置されてしまう
- [ ] テンプレート作成で、2点目のクリックで矩形が固定されないバグがある
- [ ] テンプレートをもっと追加する
- [ ] サンプルマップを3つほど追加する。それぞれコンセプトが異なるものにする
- [ ] 3Dプレビュー機能のバグ：
   - [ ] 初期表示時にフロアが奥側に表示されており、中心からズレている
   - [ ] ズームイン、ズームアウト、回転といったボタンが壊れている
   - [ ] カスタム床、カスタム壁の色が反映されない
